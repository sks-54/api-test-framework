...
def cmd_run(argv: list[str] | None = None) -> None:
    ...
    if resolved_key:
        ai_label = "explicit --api-key" if key_source == "explicit" else "ANTHROPIC_API_KEY (auto-detected)"
        ...
        print("[apitf-run] AI mode   : none — no key found (checked --api-key and ANTHROPIC_API_KEY)", flush=True)
        print("[apitf-run]             Test stub: 5-test baseline. Reflector: skipped.", flush=True)
        print("[apitf-run]             Pass --api-key or set ANTHROPIC_API_KEY to enable full suite.", flush=True)
